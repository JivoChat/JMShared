...
public struct MessageContentHash {
    public let ID: Int
    public let value: Int
    
    public func hasChanged(relatedTo anotherHash: MessageContentHash?) -> Bool {
...
public struct MessageReactor: Codable {
...
public struct MessageReaction: Codable {
    ...
    public var reactors: [MessageReactor]
    
    public init(emoji: String, reactors: [MessageReactor]) {
        self.emoji = emoji
...
public extension JVMessage {
    ...
    var direction: MessageDirection {
        ...
        else if call?.type == MessageBodyCallType.incoming {
            return .incoming
        }
        else if _isIncoming {
            return .incoming
...
//            assertionFailure()
    ...
    var contentHash: MessageContentHash {
        var hasher = Hasher()
    ...
    var call: MessageBodyCall? {
        return _body?.call
    }

    var order: MessageBodyOrder? {
        return _body?.order
    }
    ...
    var updatedMeta: MessageUpdateMeta? {
        guard let agent = _updatedAgent else { return nil }
        let date = Date(timeIntervalSince1970: _updatedTimepoint)
        return MessageUpdateMeta(agent: agent, date: date)
    }
    
    var reactions: [MessageReaction] {
        guard
            let data = _reactions,
            let items = try? PropertyListDecoder().decode([MessageReaction].self, from: data)
            else { return [] }
