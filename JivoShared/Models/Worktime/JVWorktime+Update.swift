...
extension _JVWorktime {
    public func performApply(inside context: JVIDatabaseContext, with change: JVBaseModelChange) {
        ...
        if let c = change as? JVWorktimeGeneralChange {
            if _agentID == 0 { _agentID = c.agentID }
            
            _timezoneID = c.timezoneID
            _timezone = context.object(_JVTimezone.self, primaryKey: _timezoneID)
        ...
        else if let c = change as? JVWorktimeTimezoneChange {
            _timezoneID = c.timezoneID ?? _timezoneID
            _timezone = context.object(_JVTimezone.self, primaryKey: _timezoneID)
        }
        else if let c = change as? JVWorktimeToggleChange {
            _enabled = c.enable
            _isDirty = true
        }
        else if let c = change as? JVWorktimeDayChange {
            storeLocalConfig(c.config, forDay: c.day)
            _isDirty = true
        }
        else if let c = change as? JVWorktimeDirtyChange {
            _isDirty = c.isDirty
        }
    }
    
    private func unpackRemoteConfig(enabled: Bool, start: String, end: String) -> JVWorktimeDayConfig {
    ...
    private func storeLocalConfig(_ config: JVWorktimeDayConfig, forDay day: JVWorktimeDay) {
...
public final class JVWorktimeGeneralChange: JVBaseModelChange, Codable {
    public var agentID: Int
...
public final class JVWorktimeTimezoneChange: JVWorktimeBaseChange {
...
public final class JVWorktimeToggleChange: JVWorktimeBaseChange {
...
public final class JVWorktimeDayChange: JVWorktimeBaseChange {
...
public final class JVWorktimeDirtyChange: JVWorktimeBaseChange {
