...
public final class DesignBook {
    ...
    public init(window: UIWindow) {
        ...
        if #available(iOS 13.0, *) {
            let style = lastStyleTraits.userInterfaceStyle.toDesignStyle
            styleObservable = BroadcastTool<DesignBookStyle>(defaultValue: style)
        }
        else {
            styleObservable = BroadcastTool<DesignBookStyle>(defaultValue: .light)
        }
...
//    }
    ...
    private func obtainColor(forUsage usage: DesignBookColorUsage) -> UIColor {
        ...
        case .triggerRegularButtonBackground: return dynamicColor(light: .alias(.brightBlue), dark: .alias(.brightBlue))
        case .triggerRegularButtonForeground: return dynamicColor(light: .alias(.white), dark: .hex(0xE0E0E0))
        case .triggerContrastButtonBackground: return dynamicColor(light: .alias(.white), dark: .alias(.white))
        case .triggerContrastButtonForeground: return dynamicColor(light: .hex(0x191919), dark: .hex(0x191919))
        case .actionButtonBackground: return dynamicColor(light: .alias(.white), dark: .alias(.black))
        case .actionPassiveButtonForeground: return dynamicColor(light: .alias(.steel), dark: .alias(.steel))
