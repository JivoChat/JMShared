...
public final class AlertableIcon: UIView {
    private let titleLabel = UILabel()
    private let iconView = UIImageView()
    
    public init(title: String, icon: UIImage?, template: Bool, iconMode: UIView.ContentMode) {
        super.init(frame: .zero)
        
        backgroundColor = DesignBook.shared.color(usage: .primaryBackground)
        layer.cornerRadius = DesignBook.shared.layout.alertRadius
        
        titleLabel.text = title
        titleLabel.textColor = DesignBook.shared.color(usage: .primaryForeground)
        ...
        iconView.tintColor = DesignBook.shared.color(usage: .primaryForeground)
...
fileprivate struct Layout {
    ...
    var iconViewFrame: CGRect {
        if let _ = iconView.image {
            ...
            let topY = titleLabel.hasText ? titleLabelFrame.maxY + innerMargin : verticalMargin
    ...
    private var titleLabelSize: CGSize {
        ...
        return titleLabel.calculateSize(for: parentWidth)
    }
    
    private var iconViewSize: CGSize {
        let limit = CGFloat(200)
        let size = iconView.size(for: limit)
...
fileprivate func obtainTitleLabelFont() -> UIFont {
    return DesignBook.shared.font(
        weight: .regular,
        category: .headline,
        defaultSizes: DesignBookFontSize(compact: 18, regular: 18),
        maximumSizes: nil
    )
}
