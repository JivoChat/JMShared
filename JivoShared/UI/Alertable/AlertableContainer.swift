...
public final class AlertableContainer: UIView {
    ...
    public func display(title: String?, icon: UIImage?, template: Bool, iconMode: UIView.ContentMode) {
        ...
        underlay.backgroundColor = DesignBook.shared.color(usage: .focusingShadow)
        ...
        if title == nil, icon == nil {
            ...
            shadowOpacity = 0.5
            subview = AlertableIcon(title: title ?? String(), icon: icon, template: template, iconMode: iconMode)
        }
        
        subview.bounds.size = CGSize(width: bounds.width * 0.8, height: 0)
        subview.bounds.size = subview.size(for: subview.bounds.width)
        subview.alpha = 0
        subview.center = bounds.center()
        addSubview(subview)
    ...
    private func backgroundAnimations(alpha: CGFloat, duration: TimeInterval) -> CAAnimationGroup {
    ...
    private func allAnimations(duration: TimeInterval) -> CAAnimationGroup {
        let alphaAnimation = CAKeyframeAnimation(keyPath: "opacity")
    ...
    private func outAnimations(duration: TimeInterval) -> CAAnimationGroup {
        let alphaAnimation = CAKeyframeAnimation(keyPath: "opacity")
